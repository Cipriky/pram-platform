...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  for (const file of files) {
    ...
    const blob = await put(`verificari/${params.id}/${numeFisier}`, file, {
      access: 'public',
    })
    ...
    saved.push({ id: poza.id, url: blob.url, numeFisier: file.name })
  }
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const poza = await prisma.pozaVerificare.findUnique({ where: { id: pozaId } })
  if (poza?.url) {
    try { await del(poza.url) } catch {}
  }
